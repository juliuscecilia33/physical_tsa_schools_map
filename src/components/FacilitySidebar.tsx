...
function FacilitySidebarInner({
  ...
  const combinedPhotos = useMemo(() => {
    ...
    if (displayFacility.review_images_analysis) {
      displayFacility.review_images_analysis.forEach((entry: any) => {
        if (entry.url && entry.usefulness_score != null) {
          reviewAnalysisMap.set(entry.url, {
    ...
    displayFacility?.review_images_analysis,
  ]);

  const { visiblePhotos, hiddenPhotos } = useMemo(() => {
    const overrides = displayFacility?.photo_visibility_overrides ?? {};
  ...
  }, [combinedPhotos, displayFacility?.photo_visibility_overrides]);

  const handleTogglePhotoVisibility = async (
    photoUrl: string,
    currentlyHidden: boolean,
  ) => {
    const activeFacility = displayFacility ?? facility;
    if (!activeFacility) return;
    const placeId = activeFacility.place_id;
    const overrides = activeFacility.photo_visibility_overrides ?? {};
          ...
          {displayFacility && viewMode === "facility" && (
            <div className="px-6 py-3 flex items-center justify-start gap-7">
              ...
              {displayFacility.email && displayFacility.email.length > 0 && (
                <motion.a
                  href={`mailto:${displayFacility.email[0]}`}
                  whileHover={{ scale: 1.08 }}
                  whileTap={{ scale: 0.95 }}
