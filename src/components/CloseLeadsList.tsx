      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center">
            <div className={`flex items-center overflow-hidden transition-all duration-300 ease-in-out ${searchExpanded ? 'w-80 mr-2' : 'w-0'}`}>
              <div className={`w-full bg-white/95 backdrop-blur-xl rounded-2xl shadow-sm border transition-all ${
                isFocused ? 'border-[#004aad]/30' : 'border-[#E8E9EB] hover:border-gray-300/50'
              }`}>
                <div className="flex items-center gap-4 px-4 py-1">
                  <Search className={`w-5 h-5 flex-shrink-0 ${isFocused ? 'text-[#004aad]' : 'text-gray-400'}`} />
                  <input
                    ref={searchInputRef}
                    type="text"
                    value={searchInput}
                    ...
                    className="flex-1 bg-transparent text-sm font-medium text-gray-900 placeholder:text-gray-400 focus:outline-none py-2"
                  />
                  {searchInput && (
                    <button
            ...
            <button
              onClick={() => {
                ...
                if (searchExpanded) { setSearchInput(''); setDebouncedQuery(''); }
              }}
              className={`inline-flex items-center justify-center w-10 h-10 rounded-xl border shadow-sm transition-colors ${
                searchExpanded || isSearchActive
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl border border-slate-200 text-slate-700 bg-white hover:bg-slate-50 transition-colors shadow-sm active:bg-slate-100 relative"
          >
