...
/**
 * GET /api/close/calls
 * Get call activities from Close CRM with optional filtering
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await closeClient.getCalls(params);

    console.log('[Close API] Fetched calls:', {
      count: response.data.length,
      has_more: response.has_more,
      filters: params,
      calls_with_recordings: response.data.filter((c) => c.recording_url).length,
    });
    ...
    console.error('[Close API] Calls fetch error:', error.message);
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fetch calls from Close API',
      },
      { status: 500 }
    );
  }
