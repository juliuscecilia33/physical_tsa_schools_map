...
/**
 * GET /api/close/leads
 * Get leads from Close CRM with optional filtering
 * Query params: _limit, _skip, status_id, date_created__gt, date_created__lt
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchParams.get('status_id')) {
      params.status_id = searchParams.get('status_id');
    }
    ...
    const response = await closeClient.getLeads(params);

    console.log('[Close API] Fetched leads:', {
    ...
    console.error('[Close API] Leads fetch error:', error.message);
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fetch leads from Close API',
      },
      { status: 500 }
    );
  }
