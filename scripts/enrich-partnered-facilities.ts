...
// Pagination limits
const MAX_PHOTOS = 40; // Stop after collecting 40 photos
const MAX_REVIEWS = 58; // Stop after collecting 58 reviews
...
/**
 * Load facilities with Partnership Active tag that haven't been scraped yet
 */
async function loadPartneredFacilities(
  limit?: number,
): Promise<FacilityToEnrich[]> {
  console.log("📂 Loading partnered facilities...");

  try {
    // Query facilities with Partnership Active tag that haven't been serp_scraped
    let query = supabase
      ...
      .select(
        ...
        location,
        serp_scraped,
        facility_tag_assignments!inner(tag_id)
      `,
      )
      .eq("facility_tag_assignments.tag_id", PARTNERSHIP_ACTIVE_TAG_ID)
      .or("serp_scraped.is.null,serp_scraped.eq.false");
    ...
    if (!data || data.length === 0) {
      console.log("✅ No partnered facilities need enrichment!");
      return [];
    }

    // Parse location from PostGIS POINT format
    const facilities: FacilityToEnrich[] = data.map((f: any) => {
      // location is in format: POINT(lng lat) or {coordinates: [lng, lat]}
      let lat = 0;
      let lng = 0;

      if (typeof f.location === "string") {
        // Parse "POINT(lng lat)" format
        const matches = f.location.match(/POINT\(([^ ]+) ([^)]+)\)/);
        if (matches) {
          lng = parseFloat(matches[1]);
          lat = parseFloat(matches[2]);
        }
      } else if (f.location?.coordinates) {
        // GeoJSON format
    ...
    console.log(
      `✅ Loaded ${facilities.length} partnered facilities to enrich\n`,
    );
    return facilities;
  } catch (error: any) {
    console.error(`❌ Error loading facilities: ${error.message}`);
  ...
  try {
    ...
    if (dataId) {
      ...
      try {
          ...
          for (let i = 0; i < originalPhotos.length; i++) {
            ...
            if (result.success && result.supabaseUrl) {
              ...
              processedPhotos.push({
                ...
                thumbnail: result.supabaseUrl, // Use same URL for thumbnail
              });
              imagesUploaded++;
        ...
        console.log(`     ⚠️  Photos API error: ${photoError.message}`);
      }
 ...
 * Main function
 */
async function enrichPartneredFacilities() {
  const args = process.argv.slice(2);
  const limitArg = args.find((arg) => arg.startsWith("--limit="));
  const limit = limitArg ? parseInt(limitArg.split("=")[1]) : undefined;

  console.log("🚀 Partnered Facilities SerpAPI Enrichment Script");
  console.log("=".repeat(70));

  if (limit) {
    console.log(`⚠️  LIMIT MODE: Processing only ${limit} facilities\n`);
  }

  console.log("\n📋 This script will:");
  console.log("   • Load facilities with 'Partnership Active' tag");
  console.log("   • Skip facilities already enriched (serp_scraped = true)");
  ...
  console.log(
    "   • Each facility requires ~10 API calls (1 place_id + ~3 photos pages + ~6 reviews pages)",
  );
  console.log("=".repeat(70) + "\n");

  // Load facilities
  const facilities = await loadPartneredFacilities(limit);

  if (facilities.length === 0) {
    console.log("\n✅ All partnered facilities are already enriched!");
    return;
  }
  ...
  console.log(
    "\n✅ All partnered facilities have been enriched with SerpAPI data!",
  );
  console.log("=".repeat(70));
}

enrichPartneredFacilities().catch((error) => {
