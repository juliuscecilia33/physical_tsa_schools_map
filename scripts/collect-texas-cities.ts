...
const PROGRESS_FILE = path.join(__dirname, "../.texas-cities-progress.json");
...
async function processCity(
  ...
  // Search all sport types in this city
  for (let i = 0; i < SPORT_TYPES.length; i++) {
    const sportType = SPORT_TYPES[i];
    const query = `${sportType} in ${city}`;

    console.log(`  [${i + 1}/${SPORT_TYPES.length}] Searching: ${sportType}...`);
...
async function collectTexasCitiesFacilities() {
  console.log("🚀 Starting Texas Cities Sports Facilities Collection");
  console.log("=" .repeat(60));
  console.log(`📊 Cities: ${TEXAS_CITIES.length} major Texas cities`);
  console.log(`🏃 Sport Types: ${SPORT_TYPES.length} types per city`);
  console.log("=" .repeat(60) + "\n");
...
// Run the collection
collectTexasCitiesFacilities().catch((error) => {
