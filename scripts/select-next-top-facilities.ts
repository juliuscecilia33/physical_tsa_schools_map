...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error("❌ Error: Missing required environment variables (NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)");
  process.exit(1);
}
...
function calculateQualityScore(facility: Facility): {
  ...
  // Photo limit bonus (15% weight) - prioritize facilities with 5+ photos
  // These are likely hitting the Google Places API limit and have more photos available
  const photoLimitBonus = photoCount >= 5 ? 1 : 0;
...
/**
 * Load the first batch of facility IDs to exclude
 */
function loadExcludedIds(): Set<string> {
  ...
  if (!fs.existsSync(firstBatchPath)) {
    console.error("❌ Error: First batch file not found:", firstBatchPath);
    console.error("   Run select-top-facilities.ts first.");
    process.exit(1);
  }

  const firstBatchData = JSON.parse(fs.readFileSync(firstBatchPath, "utf-8"));
  const ids = new Set<string>(
    firstBatchData.facilities.map((f: any) => f.id)
  );

  console.log(`📋 Loaded ${ids.size} facility IDs to exclude (first batch)\n`);
  return ids;
}
 ...
 */
async function fetchAndScoreFacilities(
  ...
  while (hasMore) {
    const { data, error } = await supabase
      ...
      .select("id, place_id, name, sport_types, identified_sports, sport_metadata, address, location, phone, website, rating, user_ratings_total, photo_references, cleaned_up, serp_scraped")
      .eq("hidden", false)
      .eq("cleaned_up", false)
  ...
  // Transform data to match Facility interface
  // Parse WKB hex (PostGIS geography) into lat/lng
  const transformedData = allFacilities.map((f: any) => {
    let lat = 0, lng = 0;
    if (f.location && typeof f.location === "string") {
      // WKB hex for POINT: skip 18-char header (byte order + type + SRID), then read X (lng) and Y (lat) as float64
      const hex = f.location;
      const buf = Buffer.from(hex, "hex");
      // Byte 0: byte order (01 = little-endian)
      const le = buf[0] === 1;
      // Bytes 1-4: type, Bytes 5-8: SRID (for EWKB), then X and Y
      const offset = hex.length >= 50 ? 9 : 5; // EWKB with SRID: 9-byte offset
  ...
  console.log(
    `✅ Filtered to ${filteredFacilities.length} facilities (rating >= 3.5, has photos, reviews <= 3000)\n`
  );
  ...
  // Exclude facilities from the first batch
  const afterExclusion = texasFacilities.filter(
    (f: any) => !excludedIds.has(f.id)
  );
  console.log(
    `🚫 Excluded first batch: ${texasFacilities.length - afterExclusion.length} removed, ${afterExclusion.length} remaining\n`
  );

  // Filter for only facilities that haven't been serp scraped yet
  const unscrapedFacilities = afterExclusion.filter(
    (f: any) => f.serp_scraped === false
  );
  console.log(
    `🔍 Filtered to ${unscrapedFacilities.length} facilities with serp_scraped = false\n`
  );

  console.log("🧮 Calculating quality scores...\n");

  // Calculate scores for all facilities
  const scoredFacilities: ScoredFacility[] = unscrapedFacilities.map(
    (facility: Facility) => {
...
function generateStatistics(facilities: ScoredFacility[]): void {
  console.log("\n" + "=".repeat(70));
  console.log("📈 FACILITY STATISTICS (Next Batch)");
  ...
  console.log(
    `   Facilities with 5+ Photos: ${facilities.filter(f => (f.photo_references?.length || 0) >= 5).length} (${((facilities.filter(f => (f.photo_references?.length || 0) >= 5).length / facilities.length) * 100).toFixed(1)}%)`
  );
  console.log(
  ...
  facilities.slice(0, 10).forEach((f, index) => {
    ...
    const photoIndicator = photoCount >= 5 ? "📸 5+" : `${photoCount}`;
    console.log(
 ...
 * Main function
 */
async function selectNextTopFacilities() {
  console.log("🚀 Next Batch: High-Quality Texas Facility Selection");
  console.log("=".repeat(70));
  console.log("🎯 Goal: Select next 2,500 Texas facilities for SerpAPI enrichment");
  console.log("\n📋 Selection Criteria:");
  ...
  console.log("   • serp_scraped = false (not yet enriched)");
  console.log("   • Excludes first batch of 2,500 facilities");
  ...
  console.log("     - 15% Photo limit bonus (5+ photos = likely more available)");
  ...
  // Load excluded IDs from first batch
  const excludedIds = loadExcludedIds();

  // Fetch and score facilities (excluding first batch + serp_scraped filter)
  const allFacilities = await fetchAndScoreFacilities(excludedIds);

  // Select top 2,500
  const topFacilities = allFacilities.slice(0, 2500);

  console.log(`✅ Selected top ${topFacilities.length} Texas facilities (next batch)\n`);

  if (topFacilities.length < 2500) {
    console.log(
      `⚠️  Note: Only ${topFacilities.length} facilities available (fewer than 2,500 target)\n`
    );
  }
  ...
  const outputPath = path.join(
    outputDir,
    "next-2500-high-quality-texas-facilities.json"
  );
  ...
  const exportData = {
    metadata: {
      ...
      batch: "next-2500",
      excluded_first_batch_count: excludedIds.size,
      selection_criteria: {
      ...
      average_rating: topFacilities.length > 0 ? (
        topFacilities.reduce((sum, f) => sum + f.rating, 0) /
        topFacilities.length
      ).toFixed(2) : "0",
      average_reviews: topFacilities.length > 0 ? Math.round(
        topFacilities.reduce((sum, f) => sum + f.user_ratings_total, 0) /
          topFacilities.length
      ) : 0,
      average_photos: topFacilities.length > 0 ? Math.round(
    ...
    facilities: topFacilities.map((f) => ({
      ...
      has_photo_limit_bonus: (f.photo_references?.length || 0) >= 5,
    })),
  };
  ...
  console.log("\n✅ Done! Next batch ready for SerpAPI enrichment.");
}

selectNextTopFacilities().catch((error) => {
