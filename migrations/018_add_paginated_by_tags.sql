-- Migration: Add paginated version of get_facilities_by_tags function
-- Purpose: Enable batch loading of priority (SerpAPI) facilities for faster initial load

-- Drop function if it exists
DROP FUNCTION IF EXISTS get_facilities_by_tags_paginated(UUID[], INTEGER, INTEGER, BOOLEAN, BOOLEAN);

-- Function: Get facilities that have ANY of the specified tags (with pagination)
...
COMMENT ON FUNCTION get_facilities_by_tags_paginated IS
'Returns facilities that have ANY of the specified tag IDs with pagination support. Used for batched loading of priority facilities (e.g., SerpAPI-scraped facilities) to improve initial page load time.';
